    ...
    <div className="min-h-screen py-12 px-4 md:px-8 bg-[#EAE7DC]">
      <div className="max-w-5xl mx-auto flex flex-col gap-12">
        <header className="text-center space-y-4">
          ...
          <p className="font-sans uppercase text-xs tracking-[0.4em] text-muted">
        ...
        <main className="grid md:grid-cols-2 gap-8">
          {FLOWERS.filter(f => ["rose", "tulip", "sunflower", "lavender"].includes(f.id)).map((flower) => {
              ...
              >
                ...
                <div className="flex justify-between items-end">
                  <div>
                    ...
                    <p className="font-sans uppercase text-[10px] tracking-widest text-muted">{flower.scientificName}</p>
                  </div>
                  <div className="text-xs italic opacity-40 font-serif">Ver Postal →</div>
                </div>

                <div className="absolute top-4 right-4 w-12 h-14 border-2 border-double border-divider opacity-20 rotate-12 flex items-center justify-center text-[6px] uppercase tracking-tighter text-center">
                  Flower<br/>Post
                </div>
              </motion.a>
            );
        ...
        <footer className="flex justify-between items-center px-4 mt-12 pt-8 border-t border-[#D1CAB0]/30">
          <p className="text-[10px] text-muted uppercase tracking-widest">© 2026 El Jardín de las Postales</p>
